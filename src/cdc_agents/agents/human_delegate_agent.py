...
class HumanDelegateBaseAgent(A2AReactAgent):
    """Base human delegate agent that can be orchestrated by different orchestration types."""

    def __init__(self, agent_config: AgentConfigProps, memory_saver: MemorySaver,
                 model_provider: ModelProvider, config_props: HumanDelegateConfigProps, orchestration_type: type):
...
@injectable()
class HumanDelegateAgent(HumanDelegateBaseAgent, DeepResearchOrchestrated):

    @injector.inject
    def __init__(self, agent_config: AgentConfigProps, memory_saver: MemorySaver,
                 model_provider: ModelProvider, config_props: HumanDelegateConfigProps):
        super().__init__(agent_config, memory_saver, model_provider, config_props, DeepResearchOrchestrated)
